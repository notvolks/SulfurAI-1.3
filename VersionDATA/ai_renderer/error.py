...
def write_error(error, type):
    ...
    folder_path_error_log = os.path.join(current_dir_e_log, 'ErrorLogs')
    ...
    folder_path_error_debug_log = os.path.join(current_dir_db_log, 'ErrorLogs', 'logs')
    ...
    folder_path_error_list_d_log = os.path.join(current_dir_d_log, 'ErrorLogs', 'logs')
...
def error(layout_type, error_type, error_message, error_code):  # Print errors
    if layout_type == "er1":
        ...
        write_error(f"Error Code ({error_code}) // No {error_type} ~{error_message} Found. **Solution - install new version**", "debug_error")
        ...
        print_verti_list(text)
        print(f"Error code: {error_code}")
        write_error(f"Error Code ({error_code}) // No {error_type} Failed. ~{error_message}. **Solution - install new version**", "debug_error")
