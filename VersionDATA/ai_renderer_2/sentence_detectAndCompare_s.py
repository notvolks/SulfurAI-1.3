...
class Infer_time_model:
    ...
    def summarize_changes(self, changes):
        lines = []
        for c in changes:
            types = ', '.join(f"{k}: {'+' if v > 0 else ''}{v * 100:.2f}%" for k, v in c['type_changes'].items())
            intents = ', '.join(f"{k}: {'+' if v > 0 else ''}{v * 100:.2f}%" for k, v in c['intent_changes'].items())
            lines.append(
    ...
    def find_average(self,changes):
        ...
        for change in changes:
            for k,v in change["type_changes"].items(): type_t_change[k] = type_t_change.get(k, 0) + v
            for k, v in change["intent_changes"].items(): intent_t_change[k] = intent_t_change.get(k, 0) + v

        avg_type_change = {k: v / count for k, v in type_t_change.items()}
        avg_intent_change = {k: v / count for k, v in intent_t_change.items()}
...
def run_model(past_d_changes, changes_d_apart_at_leastDays, granularity='day'):
    ...
    changes_summarised = model.summarize_changes(recent)
    average_summary_fixed = "\n\n" + average_summary if changes_summarised else None

    return changes_summarised,average_summary_fixed
