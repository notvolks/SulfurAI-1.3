...
with open(file_path_autotrainer_debug, "r", encoding="utf-8", errors="ignore") as file:  extra_debug_autotrainer = ",".join(file.readlines())
...
while True:
    ...
    file_path_wait_extra = call.settings_auto_trainer_delay()
    with open(file_path_wait_extra, "r", encoding="utf-8", errors="ignore") as file: delay_seconds = int(file.readline())
    print(f"###################--------- SulfurAI has been given a {delay_seconds} second delay. Waiting now.... [Change this value in the settings]")
